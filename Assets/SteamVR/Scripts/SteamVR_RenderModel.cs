        ...
        private IEnumerator SetModelAsync(string newRenderModelName)
        {
            ...
            using (var holder = new RenderModelInterfaceHolder())
            {
                ...
                if (count > 0)
                {
                    ...
                    {
                        var capacity = renderModels.GetComponentName(newRenderModelName, (uint)componentIndex, null, 0);
                        if (capacity == 0)
                            continue;
                ...
                {
                    var loading = false;
                    for (var renderModelNameIndex = 0; renderModelNameIndex < renderModelNames.Length; renderModelNameIndex++)
                    {
