...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Hand : MonoBehaviour
    {
        ...
        public void AttachObject(GameObject objectToAttach, GrabTypes grabbedWithType, AttachmentFlags flags = defaultAttachmentFlags, Transform attachmentOffset = null)
        {
            ...
            if (attachedObject.attachedRigidbody != null)
            {
                if (attachedObject.interactable.attachedToHand != null) //already attached to another hand
                {
                    ...
                    for (var attachedIndex = 0; attachedIndex < attachedObject.interactable.attachedToHand.attachedObjects.Count; attachedIndex++)
                    {
                        var attachedObjectInList = attachedObject.interactable.attachedToHand.attachedObjects[attachedIndex];
            ...
            if (attachedObject.HasAttachFlag(AttachmentFlags.SnapOnAttach))
            {
                if (attachmentOffset != null)
                {
                    ...
                    var rotDiff = Quaternion.Inverse(attachmentOffset.transform.rotation) * objectToAttach.transform.rotation;
            ...
            else
            {
                if (attachmentOffset != null)
                {
                    ...
                    var posDiff = (rotationPositionBy * objectToAttach.transform.position) - (rotationPositionBy * attachmentOffset.transform.position);
        ...
        protected virtual void UpdateHovering()
        {
            ...
            if (useHoverSphere)
            {
                var scaledHoverRadius = hoverSphereRadius * Mathf.Abs(SteamVR_Utils.GetLossyScale(hoverSphereTransform));
            ...
            {
                var scaledHoverRadius = controllerHoverRadius * Mathf.Abs(SteamVR_Utils.GetLossyScale(this.transform));
            ...
            {
                var scaledHoverRadius = fingerJointHoverRadius * Mathf.Abs(SteamVR_Utils.GetLossyScale(this.transform));
        ...
        protected virtual bool CheckHoveringForTransform(Vector3 hoverPosition, float hoverRadius, ref float closestDistance, ref Interactable closestInteractable, Color debugColor)
        {
            ...
            var numColliding = Physics.OverlapSphereNonAlloc(hoverPosition, hoverRadius, overlappingColliders, hoverLayerMask.value);
        ...
        protected virtual void HandFollowUpdate()
        {
            ...
            if (attachedObject != null)
            {
                if (currentAttachedObjectInfo.Value.interactable != null && currentAttachedObjectInfo.Value.interactable.handFollowTransform != null)
                {
                    ...
                    {
                        var offset = Quaternion.Inverse(this.transform.rotation) * currentAttachedObjectInfo.Value.handAttachmentPointTransform.rotation;
                        var targetHandRotation = currentAttachedObjectInfo.Value.interactable.handFollowTransform.rotation * Quaternion.Inverse(offset);
                    ...
                    {
                        var worldOffset = (this.transform.position - currentAttachedObjectInfo.Value.handAttachmentPointTransform.position);

                        var rotationDiff = mainRenderModel.GetHandRotation() * Quaternion.Inverse(this.transform.rotation);

                        var localOffset = rotationDiff * worldOffset;
                        var targetHandPosition = currentAttachedObjectInfo.Value.interactable.handFollowTransform.position + localOffset;
        ...
        protected void UpdateAttachedVelocity(AttachedObject attachedObjectInfo)
        {
            ...
            var targetItemPosition = currentAttachedObjectInfo.Value.handAttachmentPointTransform.TransformPoint(attachedObjectInfo.initialPositionalOffset);
            ...
            var targetItemRotation = currentAttachedObjectInfo.Value.handAttachmentPointTransform.rotation * attachedObjectInfo.initialRotationalOffset;
            var rotationDelta = targetItemRotation * Quaternion.Inverse(attachedObjectInfo.attachedObject.transform.rotation);
        ...
        {
            var seconds = (float)microSecondsDuration / 1000000f;
        ...
        private void InitController()
        {
            ...
            var renderModelInstance = GameObject.Instantiate(renderModelPrefab);
