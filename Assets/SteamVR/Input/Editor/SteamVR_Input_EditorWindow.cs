...
#pragma warning disable 0219 // variable assigned but not used.
    public class SteamVR_Input_EditorWindow : EditorWindow
    {
        ...
        {
            var copy = UnityEditor.EditorUtility.DisplayDialog("Copy Examples", "It looks like your project is missing an actions.json. Would you like to use the example file?", "Yes", "No");
            if (copy)
            {
        ...
        private void UpdateProgressBar()
        {
            ...
            {
                var cancel = EditorUtility.DisplayCancelableProgressBar(progressBarTitle, progressBarText, progressBarAmount);
                if (cancel)
                {
        ...
        private void OpenControllerBindings()
        {
            if (HasBeenModified())
            {
                var saveFirst = EditorUtility.DisplayDialog("Save?", "It looks like you've made changes without saving. Would you like to save before editing the bindings?.", "Save", "Open without saving");
                if (saveFirst)
                {
                    SaveFile();
                }
        ...
        private void OnDestroy()
        {
            if (HasBeenModified())
            {
                var saveFirst = EditorUtility.DisplayDialog("Save?", "It looks like you've closed the input actions window without saving changes. Would you like to save first?", "Save", "Close");
                if (saveFirst)
                {
                    SaveFile();
                }
        ...
        private void DrawDetails()
        {
            ...
            if (selectedActionIndex != -1)
            {
                ...
                if (selectedAction.type == SteamVR_Input_ActionFile_ActionTypes.skeleton)
                {
                    ...
                    var selectedSkeletonType = Array.IndexOf(SteamVR_Input_ActionFile_ActionTypes.listSkeletons, currentSkeletonPath);
                    var newSelectedSkeletonType = EditorGUILayout.Popup(selectedSkeletonType, SteamVR_Input_ActionFile_ActionTypes.listSkeletons);
            ...
            if (selectedActionIndex != -1)
            {
                ...
                var newRequirement = GUILayout.SelectionGrid(oldRequirement, SteamVR_Input_ActionFile_Action.requirementValues, 1, EditorStyles.radioButton);
            ...
            {
                var localizationItems = SteamVR_Input.actionFile.localizationHelperList[selectedLocalizationIndex].items;
                var oldValue = "";
        ...
        private void DrawSets()
        {
            ...
            for (var actionSetIndex = 0; actionSetIndex < SteamVR_Input.actionFile.action_sets.Count; actionSetIndex++)
            {
                if (selectedActionSet == actionSetIndex)
                {
                    ...
                    var selectedUsage = -1;
                    for (var valueIndex = 0; valueIndex < SteamVR_Input_ActionFile_ActionSet_Usages.listValues.Length; valueIndex++)
                    {
                    ...
                    if (removeSet)
                    {
                        var confirm = EditorUtility.DisplayDialog("Confirmation", "Are you sure you want to delete this action set and all of its actions?.", "Delete", "Cancel");
                        if (confirm)
                        {
        ...
        private void SaveFile()
        {
            ...
            var json = JsonConvert.SerializeObject(SteamVR_Input.actionFile, Formatting.Indented, new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore });
    ...
    public class SteamVR_Input_SettingsEditor : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (delete)
            {
                var confirm = EditorUtility.DisplayDialog("Confirmation", "Are you sure you want to delete the input code files? This may make your project unable to compile.", "Delete", "Cancel");
                if (confirm)
            ...
            if (deleteActions)
            {
                var confirm = EditorUtility.DisplayDialog("Confirmation", "Are you sure you want to delete the input action files? This will remove all references in your project to these actions.", "Delete", "Cancel");
                if (confirm)
