...
#pragma warning disable 0219 // variable assigned but not used.
    ...
    public static class SteamVR_Input_Generator
    {
        ...
        private static void ForceAssetDatabaseRefresh()
        {
            ...
            var steamVRInputImporter = ((MonoImporter)MonoImporter.GetAtPath(path));
            ...
            for (var sceneIndex = 0; sceneIndex < EditorBuildSettings.scenes.Length; sceneIndex++)
            {
                if (EditorBuildSettings.scenes[sceneIndex].enabled)
                {
                    ...
                    if (string.IsNullOrEmpty(scenePath) == false)
                    {
                        ...
                        {
                            var sceneAsset = AssetDatabase.LoadAssetAtPath(scenePath, typeof(UnityEngine.Object));
                            if (sceneAsset != null)
                            {
        ...
        private static bool ShouldAssignDefaults(MonoBehaviour behaviour)
        {
            ...
            {
                var containsProperty = behaviourType.GetProperties().Any(prop => prop.IsDefined(defaultInputActionType, false));
                var containsField = behaviourType.GetFields().Any(field => field.IsDefined(defaultInputActionType, false));
            ...
            for (var fileIndex = 0; fileIndex < files.Length; fileIndex++)
            {
                ...
                if (file.Name.EndsWith(".asset") || file.Name.EndsWith(".asset.meta") || file.Name.EndsWith(".cs") || file.Name.EndsWith(".cs.meta"))
                {
                    ...
                    if (isSet == false && isAction == false)
                    {
                        ...
                        var confirm = EditorUtility.DisplayDialog("Delete unused file", "Would you like to delete the unused input file: " + file.Name + "?", "Delete", "No");
                        if (confirm)
                        {
                            file.Delete();
                        }
        ...
        private static SteamVR_ActionSet CreateScriptableActionSet(SteamVR_Input_ActionFile_ActionSet actionSet)
        {
            ...
            var setType = typeof(SteamVR_Action).Assembly.GetType(typeof(SteamVR_ActionSet).Namespace + "." + GetSetClassName(actionSet));
        ...
        private static SteamVR_Action_List CreateScriptableActionList(SteamVR_Input_ActionFile_ActionSet fileActionSet, SteamVR_ActionSet actionSetAsset, SteamVR_ActionDirections direction)
        {
            ...
            var listType = typeof(SteamVR_Action).Assembly.GetType(typeof(SteamVR_Action).Namespace + "." + listClassName);
        ...
        public static void DeleteActionClassFiles()
        {
            ...
            var folderPath = GetSubFolderPath();
            var confirm = EditorUtility.DisplayDialog("Confirmation", "Are you absolutely sure you want to delete all code files in " + folderPath + "?", "Delete", "Cancel");
            if (confirm)
            {
        ...
        private static void GenerateActionHelpers(string actionsClassFileName)
        {
            ...
            var skipStateUpdatesParameter = new CodeParameterDeclarationExpression(new CodeTypeReference(typeof(bool)), skipStateUpdatesParamName);

            var initializeMethod = CreateStaticMethod(inputClass, SteamVR_Input_Generator_Names.initializeActionsMethodName);
            ...
            var actionsVibrationArray = new CodeArrayCreateExpression(new CodeTypeReference(typeof(SteamVR_Action_Vibration)));

            var actionsPoseArray = new CodeArrayCreateExpression(new CodeTypeReference(typeof(SteamVR_Action_Pose)));

            var actionsSkeletonArray = new CodeArrayCreateExpression(new CodeTypeReference(typeof(SteamVR_Action_Skeleton)));

            var actionsBooleanArray = new CodeArrayCreateExpression(new CodeTypeReference(typeof(SteamVR_Action_Boolean)));

            var actionsSingleArray = new CodeArrayCreateExpression(new CodeTypeReference(typeof(SteamVR_Action_Single)));

            var actionsVector2Array = new CodeArrayCreateExpression(new CodeTypeReference(typeof(SteamVR_Action_Vector2)));

            var actionsVector3Array = new CodeArrayCreateExpression(new CodeTypeReference(typeof(SteamVR_Action_Vector3)));

            var actionsNonPoseNonSkeletonArray = new CodeArrayCreateExpression(new CodeTypeReference(typeof(SteamVR_Action_In)));


            //add the getaction method to
            var actionsInitMethod = CreateStaticMethod(inputClass, SteamVR_Input_Generator_Names.initializeInstanceActionsMethodName);


            var initializeActionMethodInfo = GetMethodInfo<SteamVR_Action>(set => set.Initialize());
            var updateActionMethodInfo = GetMethodInfo<SteamVR_Action_In>(set => set.UpdateValue(SteamVR_Input_Sources.Any));
            ...
            for (var actionIndex = 0; actionIndex < SteamVR_Input.actionFile.actions.Count; actionIndex++)
            {
                ...
                var initializeActionMethod = AddStaticInvokeToMethod(initializeMethod, codeFriendlyInstanceName, initializeActionMethodInfo.Name);
            }
        ...
        private static void GenerateActionSetsHelpers(string actionSetsClassFileName)
        {
            ...
            var initializeMethod = CreateStaticMethod(inputClass, SteamVR_Input_Generator_Names.initializeActionSetsMethodName);

            var actionSetsInitMethod = CreateStaticMethod(inputClass, SteamVR_Input_Generator_Names.initializeInstanceActionSetsMethodName);
            ...
            for (var actionSetIndex = 0; actionSetIndex < SteamVR_Input.actionFile.action_sets.Count; actionSetIndex++)
            {
                ...
                var setType = typeof(SteamVR_ActionSet).Assembly.GetType(typeof(SteamVR_Input).Namespace + "." + setTypeName);
                ...
                var initializeActionMethod = AddStaticInvokeToMethod(initializeMethod, shortName, initializeActionSetMethodInfo.Name);
            }
        ...
        {
            var invokeMethod = new CodeMethodInvokeExpression(new CodeMethodReferenceExpression(new CodeTypeReferenceExpression(typeof(SteamVR_Input_References).Name), referenceGetActionName));
        ...
        {
            var invokeMethod = new CodeMethodInvokeExpression(new CodeMethodReferenceExpression(new CodeTypeReferenceExpression(typeof(SteamVR_Input_References).Name), referenceGetActionSetName));
        ...
        {
            var stringEqualsMethodInfo = GetMethodInfo<string>(set => string.Equals(null, null, StringComparison.CurrentCultureIgnoreCase));
            ...
            var stringCompare = new CodeMethodInvokeExpression(stringType, stringEqualsMethodInfo.Name, pathName, actionName, caseInvariantName);
            var returnAction = new CodeMethodReturnStatement(new CodeFieldReferenceExpression(new CodeTypeReferenceExpression(typeof(SteamVR_Input)), returnActionName));
