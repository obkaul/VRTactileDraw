...
namespace Valve.VR
{
    public partial class SteamVR_Input
    {
        ...
        public static void IdentifyActionsFile()
        {
            ...
            var fullPath = System.IO.Path.Combine(currentPath, SteamVR_Settings.instance.actionsFilePath);
        ...
        protected static void UpdatePoseActions(SteamVR_Input_Sources inputSource, bool skipStateAndEventUpdates = false)
        {
            ...
            {
                var action = actionsPose[actionIndex] as SteamVR_Action_Pose;
        ...
        protected static void UpdateSkeletonActions(SteamVR_Input_Sources inputSource, bool skipStateAndEventUpdates = false)
        {
            ...
            {
                var action = actionsSkeleton[actionIndex] as SteamVR_Action_Skeleton;
        ...
        public static void UpdateNonVisualActions(SteamVR_Input_Sources inputSource)
        {
            ...
            for (var actionSetIndex = 0; actionSetIndex < actionSets.Length; actionSetIndex++)
            {
                ...
                if (set.IsActive())
                {
                    ...
                    {
                        var actionIn = set.nonVisualInActions[actionIndex] as SteamVR_Action_In;
        ...
        public static SteamVR_ActionSet[] GetActionSets()
        {
            ...
            {
#if UNITY_EDITOR
                var assetGuids = UnityEditor.AssetDatabase.FindAssets("t:" + typeof(SteamVR_ActionSet).FullName);
                if (assetGuids.Length > 0)
                {
                    var assets = new SteamVR_ActionSet[assetGuids.Length];
                    for (var assetIndex = 0; assetIndex < assets.Length; assetIndex++)
                    {
                        var assetPath = UnityEditor.AssetDatabase.GUIDToAssetPath(assetGuids[assetIndex]);
        ...
        public static T[] GetActions<T>() where T : SteamVR_Action
        {
            ...
            {
#if UNITY_EDITOR
                var assetGuids = UnityEditor.AssetDatabase.FindAssets("t:" + type.FullName);
                if (assetGuids.Length > 0)
                {
        ...
        public static bool HasFileInMemoryBeenModified()
        {
            ...
            var newJSON = JsonConvert.SerializeObject(SteamVR_Input.actionFile, Formatting.Indented, new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore });
        ...
        private static void CheckSetup()
        {
            if (checkingSetup == false && (SteamVR_Input_References.instance.actionSetObjects == null || SteamVR_Input_References.instance.actionSetObjects.Length == 0 || SteamVR_Input_References.instance.actionSetObjects.Any(set => set != null) == false))
            {
                ...
                var open = UnityEditor.EditorUtility.DisplayDialog("[SteamVR]", "It looks like you haven't generated actions for SteamVR Input yet. Would you like to open the SteamVR Input window?", "Yes", "No");
                if (open)
                {
