...
namespace Scripts.Layout
{
    public class FancyScrollView<TData, TContext> : MonoBehaviour where TContext : class
    {
...
#if UNITY_EDITOR
        private float prevCellInterval, prevCellOffset;
        ...
        protected void UpdatePosition(float position)
        {
            ...
            for (var pos = firstCellPosition; pos <= 1f; pos += cellInterval, count++)
            {
            ...
            for (var pos = firstCellPosition; pos <= 1f; count++, pos += cellInterval)
            {
