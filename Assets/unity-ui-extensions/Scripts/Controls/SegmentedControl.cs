...
namespace Scripts.Controls
{
    ...
    public class SegmentedControl : UIBehaviour
    {
        ...
        public void LayoutSegments()
        {
            ...
            var width = (transform.rect.width / segments.Length) - (SeparatorWidth * (segments.Length - 1));

            for (var i = 0; i < segments.Length; i++)
            {
                var insetX = ((width + SeparatorWidth) * i);
                ...
                if (separator && i > 0)
                {
                    ...
                    var sep = (sepTransform != null) ? sepTransform.GetComponent<Graphic>() : (GameObject.Instantiate(separator.gameObject) as GameObject).GetComponent<Graphic>();
...
// TODO: maybe adjust text position
        ...
        ISelectHandler, IDeselectHandler
    {
        ...
        {
            var tintColor = selected ? segmentControl.selectedColor : button.colors.normalColor;
            var textColor = selected ? button.colors.normalColor : this.textColor;
            var transitionSprite = selected ? button.spriteState.pressedSprite : null;
            var triggerName = selected ? button.animationTriggers.pressedTrigger : button.animationTriggers.normalTrigger;
